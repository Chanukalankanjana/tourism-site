...
export default function Packages() {
  ...
  const handleBookNow = (packageName: string) => {
    const num = t(`whatsapp.phoneNumber.${currentLanguage.code}`)
    const baseMsg = t("whatsapp.message.packages")
    const fullMsg = `${baseMsg}\n\nTour: ${packageName}`
    window.open(`https://wa.me/${num}?text=${encodeURIComponent(fullMsg)}`, "_blank")
  }

  // Show 3 one-day tours on the home Packages section
  const packages = [
    {
      id: "23",
      name: t("packages.tours.kandyDay.name"),
      duration: "1 DAY TOUR",
    ...
    {
      id: "24",
      name: t("packages.tours.dutchFort.name"),
      duration: "1 DAY TOUR",
    ...
    {
      id: "25",
      name: t("packages.tours.mountElla.name"),
      duration: "1 DAY TOUR",
                ...
                <div className="p-6">
                  ...
                  <div className="space-y-3">
                    <button
                      ...
                      className="w-full py-3 bg-emerald-600 hover:bg-emerald-700 text-white font-semibold rounded-lg transition-colors duration-300 flex items-center justify-center space-x-2"
                    >
                      <span>{t("packages.bookNow")}</span>
                      <ArrowRight className="w-4 h-4" />
                    </button>
                    <button
                      onClick={() => navigateToTourDetail(pkg.id)}
                      className="w-full border border-gray-300 hover:border-emerald-500 text-gray-700 hover:text-emerald-600 py-3 rounded-lg font-semibold transition-colors duration-300"
                    >
